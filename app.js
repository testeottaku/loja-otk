...
var PRODUCTS = window.PRODUCTS || [];
  ...
  // Ajusta a view inicial baseado na página
  state.view = (PAGE === 'checkout') ? 'checkout' : (PAGE === 'produto' ? 'product' : 'home');

  // --- Persistência do carrinho (LocalStorage) ---
  const CART_KEY = 'otk_cart_v1';
  const COUPON_KEY = 'otk_coupon_v1';
  ...
  function loadCartFromStorage() {
    try {
      ...
      if (rawCoupon) {
        ...
        if (c && typeof c === 'object') state.coupon = { code: c.code || '', discount: Number(c.discount || 0) || 0 };
      }
    } catch (e) {
  ...
  function saveCartToStorage() {
    try {
      ...
      localStorage.setItem(COUPON_KEY, JSON.stringify(state.coupon || { code:'', discount:0 }));
    } catch (e) {}
  }
  ...
  window.addEventListener('catalog-loaded', () => { try { ensureProductSelectedIfNeeded(); } catch(_){} });
  window.addEventListener('DOMContentLoaded', () => { try { ensureProductSelectedIfNeeded(); } catch(_){} });
...
// Inicializar ícones estáticos
    ...
    function getProductIdFromUrl() {
      try {
        ...
        const m = window.location.pathname.match(/\/produto\/([^\/]+)/i);
    ...
    function ensureProductSelectedIfNeeded() {
      if (PAGE !== 'produto') return;

      const pid = getProductIdFromUrl() || (() => {
        try { return sessionStorage.getItem('otk_last_product'); } catch(_) { return null; }
      })();

      if (!pid) {
        state.catalogError = 'Produto não encontrado (sem ID na URL).';
        state.view = 'product';
        renderApp();
        return;
      }
      ...
      if (!p) {
        state.catalogError = 'Produto não encontrado. Ele pode ter sido removido.';
        state.view = 'product';
        renderApp();
        return;
      }
    ...
    function changeView(view, product = null) {
      // Navegação entre páginas (mantém o design mas separa em telas)
      if (view === 'home') {
      ...
      if (view === 'product') {
        if (product && product.id) {
          try { sessionStorage.setItem('otk_last_product', String(product.id)); } catch(_) {}
          saveCartToStorage();
          window.location.href = '/produto/' + encodeURIComponent(product.id);
          return;
        }
        window.location.href = '/';
        return;
      ...
      // Caso algum trecho ainda use views internas
      state.view = view;
      if (product) {
    ...
    // Expor funções usadas no HTML (onclick/oninput)
