...
// --- Helpers (precisam existir antes do normalizeProduct) ---
...
var PRODUCTS = window.PRODUCTS || [];
  ...
  const LS_CART_KEY = 'otk_cart_v1';
  const LS_COUPON_KEY = 'otk_coupon_v1';
  ...
  function loadCartFromStorage() {
    ...
    return arr
      .map(it => ({ ...it, quantity: Math.max(1, Number(it.quantity || 1)), unitPrice: Number(it.unitPrice ?? it.price ?? 0) }))
  ...
  function saveCartToStorage() {
    try {
      ...
      localStorage.setItem(LS_COUPON_KEY, JSON.stringify(state.coupon || { code: '', discount: 0 }));
    } catch (e) {
      // ignorar
    }
  }
  ...
  function loadCouponFromStorage() {
    ...
    return { code: String(c.code || ''), discount: (discount > 0 && discount < 1) ? discount : 0 };
  }
  ...
  function routeCheckout() {
    return isFileProtocol() ? './checkout.html' : '/checkout';
  }
  function routeProduct(id) {
    const safe = encodeURIComponent(String(id));
    return isFileProtocol() ? `./produto.html?id=${safe}` : `/produto/${safe}`;
  }
  ...
  function initStaticIcons() {
    ...
    set('icon-instagram', Icons.instagram);
    set('icon-bag', Icons.bag);
    set('icon-search', Icons.search);
    set('icon-sliders', Icons.sliders);
    set('ft-icon-insta', Icons.instagram);
    set('ft-icon-phone', Icons.phone);
    set('cb-icon-bag', Icons.bag);
    const cbx = document.getElementById('cb-icon-x');
    if (cbx) cbx.innerHTML = Icons.x;
  ...
  function initApp() {
    ...
    // recuperar carrinho + cupom
    state.cart = loadCartFromStorage();
    state.coupon = loadCouponFromStorage();

    // definir tela atual
    const page = document.body?.dataset?.page || 'home';
    if (page === 'home') state.view = 'home';
    if (page === 'checkout') state.view = 'checkout';
    if (page === 'produto') state.view = 'product';
    ...
    // se for página do produto, cria placeholder até o catálogo chegar
    if (state.view === 'product' && !state.selectedProduct) {
      const pid = getProductIdFromUrl();
      if (pid) {
        state.selectedProduct = { id: pid, name: 'Carregando...', price: 0, images: ['https://images.unsplash.com/photo-1523275335684-37898b6baf30?auto=format&fit=crop&q=80&w=600'], colors: [], sizes: [], hasSizes: false, category: '', anime: '', description: '' };
      }
    }
  ...
  window.addEventListener("catalog-loaded", (ev) => {
    // Se estiver na página do produto, tenta selecionar pelo id da URL
    try {
      if (document.body?.dataset?.page === 'produto') {
        const pid = getProductIdFromUrl ? getProductIdFromUrl() : '';
        if (pid) {
...
// Controle de Scroll (mantém a barra de pesquisa fixa)
    ...
    function changeView(view, product = null) {
      // Agora as telas são páginas separadas
      if (view === 'home') {
        window.location.href = routeHome();
        return;
      }
      if (view === 'checkout') {
        window.location.href = routeCheckout();
        return;
      }
      if (view === 'product') {
        const p = product || state.selectedProduct;
        if (!p || !p.id) return;
        window.location.href = routeProduct(p.id);
        return;
      }
    }
      const subHeader = document.getElementById('sub-header');
