...
// Auto-generated split build for Vercel multi-page
  ...
  function initStaticUI() {
    ...
    setHTML('icon-instagram', Icons.instagram);
    setHTML('icon-bag', Icons.bag);
    setHTML('icon-search', Icons.search);
    setHTML('icon-sliders', Icons.sliders);
    setHTML('ft-icon-insta', Icons.instagram);
    setHTML('ft-icon-phone', Icons.phone);
    setHTML('cb-icon-bag', Icons.bag);
    setHTML('cb-icon-x', Icons.x);
    setText('current-year', new Date().getFullYear());

    // Scroll: mantém sub-header "vivo" na home
    window.addEventListener('scroll', () => {
      const subHeader = document.getElementById('sub-header');
      if (!subHeader) return;
      if (state.view === 'home') {
        subHeader.classList.remove('max-h-0', 'opacity-0', 'pointer-events-none', 'hidden');
  ...
  // --- PERSISTÊNCIA (para carrinho não sumir ao trocar de página) ---
  const OTK_STORAGE_KEY = "otk_store_state_v1";

  function loadPersistedState() {
    try {
      const raw = localStorage.getItem(OTK_STORAGE_KEY);
      if (!raw) return;
      const saved = JSON.parse(raw);
      if (saved && typeof saved === "object") {
        if (Array.isArray(saved.cart)) state.cart = saved.cart;
        if (saved.coupon && typeof saved.coupon === "object") state.coupon = { ...state.coupon, ...saved.coupon };
  ...
  function persistState() {
    try {
      ...
      localStorage.setItem(OTK_STORAGE_KEY, JSON.stringify(payload));
    } catch (_) {}
  }

  // --- NAVEGAÇÃO (URLs compartilháveis) ---
  ...
  function getProductIdFromUrl() {
    const path = normPath(window.location.pathname);
    // /produto/<id> ou /produto/<id>/
    const m = path.match(/^\/produto\/?([^\/]+)?\/?$/);
    const idFromPath = m && m[1] ? decodeURIComponent(m[1]) : "";
  ...
  loadPersistedState();
  state.__page = getPageFromBody();
  if (state.__page === "checkout") state.view = "checkout";
  if (state.__page === "product") state.view = "product";
  state.__productId = (state.__page === "product") ? getProductIdFromUrl() : "";
  ...
  function nextHomePage(maxPages) {
    ...
    // Expor funções e estado para handlers inline (HTML)
    window.state = state;
  ...
  window.addEventListener("catalog-loaded", (ev) => {
    ...
    // Ajusta view pela página atual
    if (state.__page === "checkout") state.view = "checkout";
    if (state.__page === "product") state.view = "product";

    // Seleciona produto pela URL (página de produto)
    if (state.__page === "product") {
      const pid = state.__productId;
      state.selectedProduct = pid ? (PRODUCTS.find(p => String(p.id) === String(pid)) || null) : null;
      state.productForm = { size: '', color: '', quantity: 1, imageIndex: 0 };
      if (!state.selectedProduct) state.catalogError = "Produto não encontrado.";
    }

    // Sub-header (barra de busca/filtro) só na home
    const subHeader = document.getElementById('sub-header');
    if (subHeader && state.__page !== "home") {
...
function toggleFilter(forceClose = false) {
    ...
    function changeView(view, product = null) {
      // Navegação multi-página (mantém URL compartilhável)
      if (view === 'home') return goHome();
      if (view === 'checkout') return goCheckout();
      if (view === 'product') {
        const pid = (product && product.id) || (state.selectedProduct && state.selectedProduct.id);
        return pid ? goProduct(pid) : goHome();
      }
      return goHome();
    }
    ...
    function updateCartUI() {
      ...
      cartItemsDiv.innerHTML = state.cart.map((item, idx) => `
          ...
          <div class="flex-grow">
            ...
            <div class="flex items-center gap-3 mt-2">
              <div class="flex items-center border border-gray-200 rounded-lg">
                <button onclick="updateCartQuantity(${idx}, -1); updateCartUI(); if(state.view==='checkout') renderApp();" class="p-1 hover:bg-gray-100 text-gray-600 rounded-l-lg">${Icons.minus}</button>
                <span class="px-2 text-sm font-bold text-gray-900">${item.quantity}</span>
                <button onclick="updateCartQuantity(${idx}, 1); updateCartUI(); if(state.view==='checkout') renderApp();" class="p-1 hover:bg-gray-100 text-gray-600 rounded-r-lg">${Icons.plus}</button>
              </div>
              <button onclick="updateCartQuantity(${idx}, -${item.quantity}); updateCartUI(); if(state.view==='checkout') renderApp();" class="text-red-500 hover:text-red-700 text-xs flex items-center gap-1 font-bold transition">${Icons.trash} Remover</button>
            </div>
          </div>
        </div>
      `).join('');

      let subtotal = state.cart.reduce((a,i)=>a+(i.price*i.quantity), 0);
